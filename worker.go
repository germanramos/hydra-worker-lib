...
func (self *lbWorker) Run(fn func([]map[string]interface{}, map[string]string) []interface{}) {
	...
	for reply := [][]byte{}; ; {
		...
		processInstances := func(levels []interface{}, ci *[]interface{}) []interface{} {
			for _, level := range levels {
				kind := reflect.TypeOf(level).Kind()
				if kind == reflect.Slice || kind == reflect.Array {
					o := make([]interface{}, 0)
					*ci = append(*ci, processMapLevels(level.([]interface{}), &o))
				} else {
					t := fn(levels, args)
					return t
				}
		...
		computedInstances := processInstances(levels, &tmpInstances)
